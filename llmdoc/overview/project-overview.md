...
- `internal/style/style_test.go`：样式 tokenizer、反引号 token、`unquote`、`parseInt`、默认样式，以及空字符串 `Unquote("")` 的回归测试
- `sorter_test.go`：`IsNumRune`、`DecimalStrcmp`、`Strcmp`、`getStringType`
- `internal/index/pagesorter_test.go`：`NewPageSorter` 页码优先级初始化，以及 `-r` 模式下按页码语义去重的回归测试
- `input_test.go`：`ScanIndexEntry`、`CompareIndexEntry`、`skipspaces`
- `output_test.go`：输出渲染边界与编码写出回归，包括组首子条目分隔符回退与 GBK 编码 flush 行为
...
- 最近几轮回归又补强了几类容易被忽略的资源与边界条件：样式层 `Unquote("")` 不再因空字符串触发越界 panic；`-r` 模式下重复页码按页码值而不是对象指针去重；`main.go` 中的 CPU profile 文件与日志文件都由入口层显式关闭。
