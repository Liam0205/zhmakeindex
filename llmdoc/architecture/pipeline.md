...
`main.go` 现在把两类可关闭资源都显式纳入入口层管理：

- `setupLog(option)` 在启用 `-t` 时返回日志文件句柄，由 `main()` 统一 `defer Close()`。
- `-cpuprofile` 打开性能分析文件后，在 `StartCPUProfile` 成功时同时注册 `defer pprof.StopCPUProfile()` 与 `defer f.Close()`；若启动 profiling 失败，则立即关闭文件后退出。

这样做的稳定语义是：

- 日志输出目标不再依赖进程退出时由操作系统回收文件句柄。
- CPU profile 的停止与文件关闭顺序固定，避免 profile 数据尚未停止写入时就提前关闭底层文件。
